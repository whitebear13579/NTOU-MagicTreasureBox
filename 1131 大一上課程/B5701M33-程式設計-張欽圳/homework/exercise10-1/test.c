...
unsigned long long move_bottle_horse(int red_bottle,int green_bottle){
    ...
    if ( record[red_bottle][green_bottle] != -1 ) record[red_bottle][green_bottle];
...
int solve( int nr , int ng ){
    ...
    record[3][0] = record[0][3] = 3;
    record[1][0] = record[0][1] = record[1][1] = 1;
    record[1][2] = record[2][1] = 2;
...
signed main(){
    int nr = 0, ng = 0;
    if ( submit ){
        while ( nr = nextint(), ng = nextint(), nr != 0 && ng != 0 ){
            solve(nr,ng);
        }
